...
/// Manages the single local Whisper model (`large-v3-turbo`, ~1.6 GB). The app downloads and
/// owns its own copy under Application Support — it does not reuse any other app's cache, so a
...
final class WhisperModelStore: NSObject {
    ...
    /// Real file is ~1,624,555,275 bytes; a 1 GB floor rejects truncated/partial files.
    private static let minValidBytes: Int64 = 1_000_000_000
    ...
    var modelsDirectory: URL? {
        ...
        return base.appendingPathComponent("RewriteDB/models", isDirectory: true)
    }
    ...
    func download() {
        ...
        if freeBytes(at: dir) < Self.requiredFreeBytes {
            status = .failed("Not enough free disk space — the model needs about 2 GB.")
            return
        }
...
extension WhisperModelStore: URLSessionDownloadDelegate {
    ...
    nonisolated private func modelURLNonisolated() -> URL? {
        ...
        return base.appendingPathComponent("RewriteDB/models/\(Self.modelFilename)", isDirectory: false)
    }
}
