...
// First-run onboarding wizard for RewriteDB.
//
...
//
// ── Integration ──────────────────────────────────────────────────────────────────────────────
// In RewriteDBApp.swift add a window scene and open it on first launch:
//
...
struct OnboardingView: View {
    ...
    private var primaryTitle: String {
        switch model.step { case .welcome: "Get started"; case .done: "Start using RewriteDB"; default: "Continue" }
    }
...
private struct WelcomeStep: View {
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("Welcome to RewriteDB").font(.title).bold()
...
private struct AccessibilityStep: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            StepTitle("Let RewriteDB work in any app",
                      "macOS Accessibility access lets RewriteDB copy your selected text and paste the result back. It’s used only for that — nothing is recorded.")
            ...
            if model.accessibilityGranted {
                ...
                StatusRow(title: "Accessibility", state: .actionNeeded, badgeLabel: "Waiting…",
                          caption: "Enable RewriteDB in the list. If macOS asks you to relaunch, the wizard resumes right here.")
            }
            Spacer()
        }
    }
...
private struct DoneStep: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Label("Select text anywhere and press ⇧⌘R. Find RewriteDB in your menu bar, up top.",
