...
extension Instruction {
    /// The current Auto Clean system prompt. (Also applied via `DictidyKit.rewriteInputMessage`, which
    /// wraps the input as inert data so a dictation phrased at the assistant gets cleaned, not answered.)
    ...
    /// The previous Auto Clean default — used to migrate an *unedited* Auto Clean prompt up to the
    /// current one, without clobbering a prompt the user has customized (exact-match guard in AppState).
