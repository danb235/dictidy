...
let package = Package(
    ...
    targets: [
        // Pure, dependency-free logic (models + API parsing). Unit-tested by DictidyTests.
        .target(
            name: "DictidyKit",
            path: "Sources/DictidyKit"
        ),
        ...
        .executableTarget(
            name: "Dictidy",
            dependencies: ["KeyboardShortcuts", "DictidyKit", "whisper", "llama"],
            path: "Sources/Dictidy"
        ),
        // Dependency-free test runner: `swift run DictidyTests`. Works under the Command
        // Line Tools (XCTest / swift-testing aren't fully available without full Xcode).
        .executableTarget(
            name: "DictidyTests",
            dependencies: ["DictidyKit"],
            path: "Sources/DictidyTests"
        )
    ]
)
