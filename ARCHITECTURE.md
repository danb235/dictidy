...
A contributor's tour of how RewriteDB is put together. For build/test instructions see
[CONTRIBUTING.md](CONTRIBUTING.md); for the user-facing overview see the [README](README.md).

## Shape

RewriteDB is a SwiftUI **menu-bar app** (`MenuBarExtra`, `LSUIElement` — no Dock icon), built as a
Swift Package (`swift build`, no `.xcodeproj`). Three layers:

```
Sources/
  RewriteDBKit/   ── pure, dependency-free logic (unit-tested; the only unit-testable target)
  RewriteDB/      ── the app
  ...
  RewriteDBTests/ ── dependency-free test runner (XCTest isn't available under CLT-only)
```

- **`RewriteDBKit`** has no platform dependencies — `Instruction`, `AnthropicModel`, `AnthropicClient`
...
`release.yml` (on a `vX.Y.Z` tag) builds + version-stamps `RewriteDB.app`, zips it, and publishes a
...
`RewriteDBKit` is unit-tested by `Sources/RewriteDBTests/main.swift` (a plain executable — XCTest isn't
